...
function getSampleBuffer(key) {
  return Buffer.from(samples[key].match(/[0-9a-f]{2}/g).map(i => parseInt(i, 16)));
}
...
describe('lib/index', () => {
  ...
  test('parse successfully handles short headers', () => {
    ...
    expect(parse(sample)).toEqual({ headers: { foo: 65535 }, payload: '' });
  });

  test('parse successfully handles integer headers', () => {
    const sample = getSampleBuffer('integerHeader');
    expect(parse(sample)).toEqual({ headers: { foo: 4294967295 }, payload: '' });
  });

  test('parse successfully handles long headers', () => {
    const sample = getSampleBuffer('longHeader');
    expect(parse(sample)).toEqual({ headers: { foo: 281474976710655 }, payload: '' });
  });
  ...
  test('parse returns a parsed object if the content-type is JSON', () => {
    ...
    expect(parse(sample)).toEqual({
      ...
      payload: {
        ...
        Records: [
          {
            ApproximateArrivalTimestamp: 1538160219.636,
            Data: 'V2VzbGV5',
            EncryptionType: null,
  ...
  test('Parser is able to transform the buffer and pipe it into an output stream', done => {
  ...
  test('Parser emits the error thrown while decoding', done => {
    const parser = new Parser();
    parser.on('error', err => {
