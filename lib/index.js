...
function parse(buffer) {
  ...
  const { length } = buffer;
  if (length < 16) throw new Error('Expected at least 16 bytes in the buffer.');
  ...
  while (offset < headersEnd) {
    ...
    switch (valueType) {
      ...
      case types.BYTE:
        value = buffer.readUInt8BE(offset);
        offset += 1;
        break;
      ...
      // Longs are 8 octects.
      case types.LONG:
        value = (buffer.readUInt32BE(offset) << 8) + buffer.readUInt32BE(offset + 4);
        offset += 8;
        break;
      ...
      case types.TIMESTAMP:
        value = new Date((buffer.readUInt32BE(offset) << 8) + buffer.readUInt32BE(offset + 4));
        offset += 8;
        break;
